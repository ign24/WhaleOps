...
Use this block first when running inside CGN-Agent. Keep the sections below as educational/reference material.

- **Phase alignment:** Applies to Phase 3 (QA assessment) in `full_analysis_protocol`.
- **Primary tools:** `run_pytest` (Python) or `run_jest` (JS/TS), plus `analyze_test_coverage`.
- **Execution order:**
  1. Detect repo language and run one test runner.
  2. Run `analyze_test_coverage`.
  3. If tests are missing or failing due to setup, report this explicitly as evidence.
- **Severity rubric:**
  - `P0`: no runnable tests in critical service, coverage effectively zero with no safety net.
  - `P1`: failing core tests or large untested critical paths.
  - `P2`: flaky/non-critical tests, low-priority coverage gaps.
- **Output contract:** Provide pass/fail summary, coverage metrics, top gaps, and prioritized next tests.
- **Note:** Script examples below are reference patterns and do not replace CGN-Agent tool calls.
