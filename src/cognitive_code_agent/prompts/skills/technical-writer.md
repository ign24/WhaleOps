...
Use this block first when running inside CGN-Agent. Keep the sections below as educational writing reference.

- **Phase alignment:** Applies to Phase 4 (Documentation review) in `full_analysis_protocol`.
- **Primary tools:** `check_readme`, `analyze_docstrings`, `analyze_api_docs`.
- **Execution order:**
  1. Run `check_readme` and assess install/usage completeness.
  2. Run `analyze_docstrings` for code-level documentation coverage.
  3. Run `analyze_api_docs` to verify OpenAPI/API documentation presence.
- **Severity rubric:**
  - `P0`: missing core setup/usage docs that block onboarding or operation.
  - `P1`: missing API documentation or major gaps in developer-facing docs.
  - `P2`: clarity, style, and structure improvements.
- **Output contract:** Report documentation gaps with affected files, user impact, and concrete updates.
- **When protocol is active:** Use these tool-backed checks before narrative writing guidance.
