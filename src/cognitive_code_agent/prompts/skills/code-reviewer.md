...
Use this block first when running inside CGN-Agent. Keep educational sections below as reference.

- **Phase alignment:** Applies primarily to Phase 1 (Code review) in `full_analysis_protocol`.
- **Primary tools:** `get_diff`, `run_ruff` (Python), `run_eslint` (JS/TS), `analyze_complexity`.
- **Execution order:**
  1. Get change/context with `get_diff` when refs are available.
  2. Run one language-appropriate linter.
  3. Run `analyze_complexity`.
  4. Cross-check top findings against touched files/components.
- **Severity rubric:**
  - `P0`: exploitable security flaw, guaranteed crash/data corruption, auth bypass.
  - `P1`: likely functional bug, high complexity hotspot in critical path, broken error handling.
  - `P2`: maintainability and style problems with lower immediate risk.
- **Output contract:** Report findings with file path, evidence, severity, and a concrete fix recommendation.
- **When protocol is active:** Follow `full_analysis_protocol` sequencing over any generic review flow.
