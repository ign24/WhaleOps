...
The system SHALL detect when assistant message content contains Python repr of LangChain message objects or internal framework state, using a set of known patterns. Detection SHALL occur on the fully accumulated content after streaming completes, not per token.
